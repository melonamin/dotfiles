...
Panel {
  ...
  // A compact world-time disclosure keeps the calendar focused on local
  // time until it is needed. The system zone database does the conversion,
  // including Berlin's daylight-saving transitions.
  property bool worldTimesExpanded: false
  property var worldTimes: [
    { city: "Berlin", time: "--:--", abbreviation: "", offset: "", date: "" },
    { city: "Moscow", time: "--:--", abbreviation: "", offset: "", date: "" },
    { city: "Dubai", time: "--:--", abbreviation: "", offset: "", date: "" }
  ]
  ...
  function toggleWorldTimes() {
    root.worldTimesExpanded = !root.worldTimesExpanded
    if (root.worldTimesExpanded) root.refreshWorldTimes()
  }

  function refreshWorldTimes() {
    if (!worldTimeProcess.running) worldTimeProcess.running = true
  }

  function applyWorldTimes(output) {
    var rows = []
    ...
    if (rows.length === 3) root.worldTimes = rows
  }
  ...
  Process {
    id: worldTimeProcess
    command: ["bash", "-c",
      "printf 'Berlin|'; TZ=Europe/Berlin date '+%H:%M|%Z|UTC%:z|%a, %b %-d'; " +
      "printf 'Moscow|'; TZ=Europe/Moscow date '+%H:%M|%Z|UTC%:z|%a, %b %-d'; " +
      "printf 'Dubai|'; TZ=Asia/Dubai date '+%H:%M|%Z|UTC%:z|%a, %b %-d'"]
  ...
  Timer {
    ...
    running: root.opened && root.worldTimesExpanded
  ...
  KeyboardPanel {
    ...
    PanelKeyCatcher {
      ...
      onTextKey: function(t) {
        ...
        else if (t === "z" || t === "Z") root.toggleWorldTimes()
      }
      ...
      Flickable {
        ...
        Column {
          ...
          // ---- World time. Collapsed by default so the stock calendar
          //      keeps its familiar proportions; click or press Z to reveal
          //      three live clocks backed by the system IANA zone database.
          Item {
            width: parent.width
            implicitHeight: worldTimeColumn.implicitHeight
            ...
            Column {
              ...
              Rectangle {
                ...
                color: worldTimeMouse.containsMouse
                ...
                Text {
                  id: worldTimeHint
                  anchors.right: worldTimeChevron.left
                  anchors.rightMargin: Style.space(9)
                  anchors.verticalCenter: parent.verticalCenter
                  text: "BERLIN · MOSCOW · DUBAI"
                ...
                Text {
                  ...
                  anchors.verticalCenter: parent.verticalCenter
                  text: root.worldTimesExpanded ? "󰅃" : "󰅀"
                  color: worldTimeMouse.containsMouse
                    ? Style.hoverStateColor(root.contentForeground, Color.accent)
                    : Qt.darker(root.contentForeground, 1.4)
                  font.family: root.contentFontFamily
                ...
                MouseArea {
                  ...
                  hoverEnabled: true
                  cursorShape: Qt.PointingHandCursor
                  onClicked: root.toggleWorldTimes()
                }

                PanelToolTip {
                  visible: worldTimeMouse.containsMouse
                  text: root.worldTimesExpanded ? "Hide world times (Z)" : "Show world times (Z)"
                  fontFamily: root.contentFontFamily
                }
              }

              Column {
                visible: root.worldTimesExpanded
                width: parent.width
                spacing: Style.space(3)

                Repeater {
                  model: root.worldTimes
                  ...
                  Rectangle {
                    ...
                    color: Qt.rgba(root.contentForeground.r, root.contentForeground.g,
                                   root.contentForeground.b, 0.045)
                    ...
                    Column {
                      ...
                      Text {
                        anchors.right: parent.right
                        text: modelData.abbreviation + (modelData.offset ? " · " + modelData.offset : "")
                        color: Qt.darker(root.contentForeground, 1.5)
                        font.family: root.contentFontFamily
                        font.pixelSize: Style.font.caption
                      }

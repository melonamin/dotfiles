...
Item {
  ...
  readonly property string usageDir: (Quickshell.env("XDG_STATE_HOME") || home + "/.local/state") + "/omarchy/multi-agents/usage"
  readonly property string collectorPath: home + "/.config/omarchy/plugins/melonamin.multi-agents/collect-multi.sh"
  ...
  function updateCommand(kind, agentIds) {
    var command = ["bash", root.collectorPath]
  ...
  function displayProvider(record) {
    ...
    return {
      ...
      providerKind: String(record.providerKind || record.id),
      iconId: String(record.iconId || record.id),
      selectorLabel: String(record.selectorLabel || record.name || record.id),
      viewKind: String(record.viewKind || "account"),
      sortOrder: Number(record.sortOrder || 999),
      launchCommand: String(record.launchCommand || ""),

...
This user-owned fork of `omarchy.agents` opens on a compact weekly-limit
overview, shows independent detailed limits for five local subscription
profiles, and keeps shared transcript history in one combined view. Overview
rows show each local sign-in email and open their matching account. Session
pressure appears as an accent dot at
75% and turns urgent at 90%; hover it for the session percentage and reset.
The bar icon turns urgent only when every configured subscription is exhausted
by its general weekly or session window.

| View | Credential home | Launcher |
|---|---|---|
| Claude 1 | `~/.claude` | `clx` |
| Claude 2 | `~/.claude2` | `clx2` |
| Combined Usage | shared Claude and Codex transcripts | — |
| Codex 1 | `~/.codex` | `cx` |
| Codex 2 | `~/.codex2` | `cx2` |
| Codex 3 | `~/.codex3` | `cx3` |

Per-account collection only probes limits. The stock Omarchy Claude and
Codex collectors each scan once to build Combined Usage, avoiding duplicate
walks over the symlinked `projects` and `sessions` directories.
Account emails are extracted locally from each CLI's account metadata. Tokens
are never written to usage records, and emails are excluded from cross-device
usage aggregation.

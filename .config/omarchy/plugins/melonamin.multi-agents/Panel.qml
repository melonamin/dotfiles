...
Panel {
  id: root
  moduleName: "melonamin.multi-agents"
  ipcTarget: "melonamin.multi-agents"
  manageIpc: false
  ...
  readonly property var accountProviders: usage.enabledProviders
  readonly property var overviewProvider: ({
    providerId: "overview",
    providerName: "Overview",
    providerKind: "overview",
    iconId: "",
    selectorLabel: "Overview",
    viewKind: "overview",
    tierLabel: "Weekly allowances",
    usageStatusText: "",
    authHelpText: "",
    limits: [],
    recentDays: [],
  ...
  readonly property var providers: [overviewProvider].concat(accountProviders)
  readonly property var overviewAccounts: accountProviders.filter(function(p) {
    return String(p.viewKind || "") === "account"
  })
  ...
  readonly property bool alarming: allSubscriptionsExhausted()
  ...
  function selectProviderById(providerId) {
    for (var i = 0; i < providers.length; i++) {
  ...
  // The overview deliberately shows the general weekly allowance, not a
  // model-scoped weekly bucket such as Fable Weekly.
  function weeklyWindow(p) {
    if (!p) return null
    var general = generalWeeklyWindow(p)
    if (general) return general
    var fallback = null
    var list = p.limits || []
    for (var i = 0; i < list.length; i++) {
      var entry = list[i] || {}
      var normalized = limitWindow(entry.label, entry.percent, entry.resetsAt, entry.title)
      if (!windowIsLong(String(entry.label || "").toLowerCase())) continue
      if (!fallback) fallback = normalized
    }
    return fallback
  }

  function generalWeeklyWindow(p) {
    if (!p) return null
    var list = p.limits || []
    for (var i = 0; i < list.length; i++) {
      var entry = list[i] || {}
      var normalized = limitWindow(entry.label, entry.percent, entry.resetsAt, entry.title)
      if (String(normalized.title || "").toLowerCase() === "weekly") return normalized
    }
    return null
  }
  ...
  function sessionWindow(p) {
    ...
    for (var i = 0; i < list.length; i++) {
      var entry = list[i] || {}
      var normalized = limitWindow(entry.label, entry.percent, entry.resetsAt, entry.title)
      if (String(normalized.title || "").toLowerCase() !== "session") continue
      if (!best || normalized.percent > best.percent) best = normalized
    }
    return best
  }

  function sessionPressure(p) {
    var session = sessionWindow(p)
    return session && session.percent >= 0.75 ? session : null
  }

  function subscriptionExhausted(p) {
    var weekly = generalWeeklyWindow(p)
    var session = sessionWindow(p)
    return (!!weekly && weekly.percent >= 1.0) || (!!session && session.percent >= 1.0)
  }

  function allSubscriptionsExhausted() {
    if (overviewAccounts.length === 0) return false
    for (var i = 0; i < overviewAccounts.length; i++) {
      if (!subscriptionExhausted(overviewAccounts[i])) return false
    }
    return true
  }
  ...
  function iconCandidatesForProvider(p, surfaceColor) {
    if (!p) return []
    var iconId = String(p.iconId || "")
    if (iconId === "") return []
    var candidates = []
    if (colorLuminance(surfaceColor || Color.background) >= 0.5)
      candidates.push(Qt.resolvedUrl("assets/" + iconId + "-light.svg"))
    candidates.push(Qt.resolvedUrl("assets/" + iconId + ".svg"))
    return candidates
  }
      ...
      Flickable {
        ...
        Column {
          ...
          Grid {
            ...
            width: parent.width
            columns: root.providers.length > 6 ? 4 : 3
          ...
          Column {
            id: overviewSection
            visible: !!root.provider && String(root.provider.viewKind || "") === "overview"
            width: parent.width
  ...
  component OverviewRow: BorderSurface {
    id: overviewRow

    property var account: null
    readonly property var weekly: root.weeklyWindow(account)
    readonly property var session: root.sessionPressure(account)
    readonly property string email: account ? String(account.email || "") : ""
    readonly property bool weeklyAlarming: !!weekly && weekly.percent >= 0.9
    readonly property bool sessionAlarming: !!session && session.percent >= 0.9
    readonly property string resetText: {
      if (!weekly) return String(account ? account.usageStatusText || "Weekly limit unavailable" : "")
      var remaining = root.resetMsFor(weekly)
      return remaining >= 0 ? "Resets in " + root.formatDuration(remaining) : "Reset time unavailable"
    }
    readonly property string sessionTooltip: {
      if (!session) return ""
      var value = "Session " + Math.round(session.percent * 100) + "%"
      var remaining = root.resetMsFor(session)
      return remaining >= 0 ? value + " · resets in " + root.formatDuration(remaining) : value
    }
    ...
    borderSpec: Border.flat(
      weeklyAlarming || sessionAlarming
    ...
    Column {
      ...
      Item {
        ...
        Text {
          ...
          anchors.left: parent.left
          anchors.verticalCenter: parent.verticalCenter
          text: overviewRow.account ? overviewRow.account.providerName : ""
          color: root.foreground
          font.family: root.fontFamily
          font.pixelSize: Style.font.body
          font.bold: true
        ...
        Rectangle {
          ...
          anchors.verticalCenter: parent.verticalCenter
          color: overviewRow.sessionAlarming ? root.urgent : Color.accent
        }
        ...
        Text {
          ...
          anchors.right: parent.right
          anchors.verticalCenter: parent.verticalCenter
          text: overviewRow.weekly ? Math.round(overviewRow.weekly.percent * 100) + "%" : "—"
          color: overviewRow.weeklyAlarming ? root.urgent : root.foreground
          font.family: root.fontFamily
          font.pixelSize: Style.font.caption
        }
      }
      ...
      Text {
        visible: overviewRow.email !== ""
        width: parent.width
        text: overviewRow.email
        color: root.dim
        font.family: root.fontFamily
        font.pixelSize: Style.font.caption
      ...
      Meter {
        visible: !!overviewRow.weekly
        width: parent.width
        value: overviewRow.weekly ? overviewRow.weekly.percent : -1
        alarming: overviewRow.weeklyAlarming
      }
    ...
    MouseArea {
      ...
      onClicked: if (overviewRow.account) root.selectProviderById(overviewRow.account.providerId)
    }

    PanelToolTip {
      visible: rowMouse.containsMouse && overviewRow.sessionTooltip !== ""
      text: overviewRow.sessionTooltip
